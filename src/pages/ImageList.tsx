...
const ImageList: React.FC =  () =>{
    ...
    //Henter data tilhørende bildene når listHasChanges endrer seg, dette skjer når bilder laster opp i firebase
    //Dette gjør jeg for å oppdatere data når det legges nye bilder til i applikasjonen
    useEffect(() =>{
        readCurrentUserImageData()
    }, [listHasChanges])
    ...
    //her oppdateres antal kolonner som skal vises i flatlisten
        ...
        files!.map( (file: any) =>{
            const imageUrls =  getImageUrl(file.name).then((imageUrl: any) =>{
                ...
                uniqueImages = [...uniqueImages, {name: file.name ,url: imageUrl}]
                    //setUrl(newUrl  => [...newUrl, {url: imageUrl} ]) 
